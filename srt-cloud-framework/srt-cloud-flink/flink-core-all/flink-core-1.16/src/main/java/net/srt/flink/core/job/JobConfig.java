...
import net.srt.flink.gateway.config.AppConfig;
...
import net.srt.flink.gateway.config.SavePointStrategy;
...
public class JobConfig {
	...
	public void setAddress(String address) {
		if (GatewayType.LOCAL.equalsValue(type) && Asserts.isNotNull(config)
				&& config.containsKey("rest.port")) {
			this.address = address;
			this.jobManageAddress = address.split(NetConstant.COLON)[0];
			this.restPort = config.get("rest.port");
			//本地固定6123
			this.jobManagePort = FlinkConstant.JOB_MANAGE_PORT;
		} else {
			this.address = address;
			this.jobManageAddress = address.split(NetConstant.COLON)[0];
			this.restPort = address.split(NetConstant.COLON)[1];
		}
	}
	...
	public void buildGatewayConfig(Map<String, Object> config) {
		...
		if (config.containsKey("flinkConfig")
			...
			gatewayConfig.getFlinkConfig().getConfiguration().put("parallelism.default", String.valueOf(parallelism));
		}
